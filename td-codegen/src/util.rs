//! Small TL-to-Rust naming and primitive-type mappings.

use std::fmt;

use td_parser::TypeExpr;

/// Reports whether `expr` uses `TDLib`'s base64-encoded bytes representation.
pub fn is_bytes(expr: &TypeExpr) -> bool {
  matches!(expr, TypeExpr::Bare("bytes"))
}

/// Reports whether `expr` is a JSON-string-encoded 64-bit integer.
pub fn is_int64(expr: &TypeExpr) -> bool {
  matches!(expr, TypeExpr::Bare("int64"))
}

/// Reports whether `expr` is a vector of JSON-string-encoded 64-bit integers.
