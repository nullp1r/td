//! Allocation-aware base64 used by generated byte-field serialization.
//!
//! Encoding supports the standard and URL-safe RFC 4648 alphabets with optional
//! padding. Decoding accepts either alphabet and padded or unpadded input, which
//! matches the forms encountered at the `TDLib` boundary. Buffer forms append to
//! caller-owned storage so hot serialization paths can reuse capacity.

/// Selects the URL-safe `-` and `_` alphabet for encoding.
pub const URL_SAFE: u8 = 1 << 0;
/// Omits trailing `=` padding during encoding.
pub const NO_PADDING: u8 = 1 << 1;

/// Encodes `input` into a newly allocated base64 string.
...
/// Decodes standard or URL-safe base64 into a newly allocated byte vector.
///
/// Returns `None` for an illegal alphabet byte or an impossible encoded length.
pub fn decode(input: &str) -> Option<Vec<u8>> {
  let mut buf = Default::default();
  decode_to(&mut buf, input).then_some(buf)
}

/// Appends the base64 encoding of `input` to `buf`.
pub fn encode_to(buf: &mut String, input: &[u8], flags: u8) {
  ...
  // `queue` retains exactly `count` unencoded low bits after each drain.
  for &byte in input {
...
/// Appends decoded bytes to `buf` and reports whether all input was valid.
///
/// On failure, bytes decoded before the invalid input remain appended. Callers
/// that require transactional behavior must decode into an empty temporary buffer.
pub fn decode_to(buf: &mut Vec<u8>, input: &str) -> bool {
  ...
  // `queue` retains exactly `count` undecoded low bits after each emitted byte.
...
/// Sextet lookup for `[standard, URL-safe]` alphabets.
...
/// Reverse lookup accepting both alphabets; `!0` marks invalid bytes.
