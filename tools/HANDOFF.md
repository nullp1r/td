...
It writes `diagnostics.txt`, continues after individual failures, verifies immutable source/config/docs against the extracted handoff manifest when present, checks the required TD schema, key UX regression invariants (including both group-help discovery paths), and SQLite database, then runs the workspace format/check/test/Clippy matrix when Cargo is installed. Cargo is offline by default so a handoff does not unexpectedly fetch dependencies. If dependency/network access is intentionally available:
