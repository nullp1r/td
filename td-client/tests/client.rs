...
async fn lifecycle() {
  let fut = async {
    ...
    let res = first_sender.send(&BadRequest).await;
    assert_matches!(res, Err(Error::Json(_)));
    let res = first_sender.send(&WrongReturn).await;
    assert_matches!(res, Err(Error::Json(_)));

    let a_detached = first_sender.clone();
    let a_detached = tokio::spawn(async move { a_detached.send(&fns::testSquareInt { x: 4 }).await });
    let b_detached = first_sender.clone();
    ...
    let (a, c) = tokio::join!(first_sender.send(&first_request), second_sender.send(&second_request));
    ...
    let res = first_sender.send(&fns::testReturnError { error: err.clone() }).await;
    ...
    let racing = racing.expect("racing client failed to start");
    racing.shutdown().await.expect("racing client failed to shut down");
    ...
    let restarted = Client::new(params("restart")).await.expect("restarted client failed to start");
