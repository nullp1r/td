...
use td_client::native::{set_log_level, set_receive_timeout};
...
async fn run(config: Config, root: &Path) -> Result<()> {
  ...
  let exercise = timeout(OPERATION_TIMEOUT, exercise(&mut client, &sender, chat_id, root, &mut messages)).await;
  let exercise = match exercise {
    Ok(result) => result,
    Err(error) => Err(error).context("live Telegram exercise timed out"),
  };
  let cleanup = timeout(CLEANUP_TIMEOUT, delete_messages(&sender, chat_id, &messages)).await;
  let cleanup = match cleanup {
    Ok(result) => result,
    Err(error) => Err(error).context("remote cleanup timed out"),
  };
  let shutdown = timeout(SHUTDOWN_TIMEOUT, client.shutdown()).await;
...
async fn exercise(client: &mut Client, sender: &Sender, chat_id: i64, root: &Path, messages: &mut Vec<i64>) -> Result<()> {
  eprintln!("discovering chat");
  discover_chat(sender, chat_id).await?;
  eprintln!("testing text send, terminal update, and edit");
  text_lifecycle(client, sender, chat_id, messages).await?;
  eprintln!("testing single-document upload progress");
  let document_id = document_progress(sender, chat_id, root, messages).await?;
  eprintln!("testing media edit through its direct response");
  edit_document(sender, chat_id, document_id, root).await?;
  eprintln!("testing ten-photo album progress");
  album_progress(sender, chat_id, root, messages).await?;
  eprintln!("testing pending-message cancellation");
  cancel_document(sender, chat_id, root, messages).await
}

async fn discover_chat(sender: &Sender, chat_id: i64) -> Result<()> {
  let Chat::chat(chat) = sender.send(&fns::getChat { chat_id }).await?;
  ensure!(chat.id == chat_id, "getChat returned chat {} instead of {chat_id}", chat.id);
  Ok(())
}

async fn text_lifecycle(client: &mut Client, sender: &Sender, chat_id: i64, messages: &mut Vec<i64>) -> Result<()> {
  let request = fns::sendMessage { chat_id, input_message_content: text_content("td-client live test"), ..Default::default() };
  let sent = sender.send_message(&request, None, None).await.context("text send failed")?;
  ...
  wait_for_send_success(client, chat_id, sent.id).await?;
  ...
  let Message::message(edited) = sender.send(&request).await.context("text edit failed")?;
...
async fn wait_for_send_success(client: &mut Client, chat_id: i64, message_id: i64) -> Result<()> {
  loop {
    let Some(update) = client.recv().await else { bail!("client closed before the terminal send update") };
...
async fn document_progress(sender: &Sender, chat_id: i64, root: &Path, messages: &mut Vec<i64>) -> Result<i64> {
  ...
  let message = {
    ...
    sender.send_message(&request, None, Some(&mut observe)).await.context("document send failed")?
  };
  ...
  download_document(sender, file.id, file.size).await?;
  Ok(message.id)
}

async fn edit_document(sender: &Sender, chat_id: i64, message_id: i64, root: &Path) -> Result<()> {
  ...
  let Message::message(message) = sender.send(&request).await.context("media edit failed")?;
...
async fn album_progress(sender: &Sender, chat_id: i64, root: &Path, sent: &mut Vec<i64>) -> Result<()> {
  ...
  let results = {
    ...
    sender.send_messages(&request, None, Some(&mut observe)).await.context("album request failed")?
  };
...
async fn cancel_document(sender: &Sender, chat_id: i64, root: &Path, sent: &mut Vec<i64>) -> Result<()> {
  let path = root.join("cancel.bin");
  create_file(&path, 64 * 1024 * 1024)?;
  let request = fns::sendMessage { chat_id, input_message_content: document_content(&path), ..Default::default() };
  let cancel = CancellationToken::new();
  cancel.cancel();
  match sender.send_message(&request, Some(&cancel), None).await {
...
async fn delete_messages(sender: &Sender, chat_id: i64, message_ids: &[i64]) -> Result<()> {
  if message_ids.is_empty() {
    return Ok(());
  }
  let request = fns::deleteMessages { chat_id, message_ids: message_ids.into(), revoke: true };
  sender.send(&request).await.context("failed to delete live-test messages")?;
  Ok(())
}
...
async fn download_document(sender: &Sender, file_id: i32, size: i64) -> Result<()> {
  ...
  for (offset, limit, total) in [(0, 0, size), (4096, 8192, 8192)] {
    ...
    let file = {
      ...
      sender.download(&request, None, Some(&mut observe)).await.context("download failed")?
    };
