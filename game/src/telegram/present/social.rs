...
fn group_content(event: &GroupEventView, world_first: bool) -> types::inputMessageRichMessage {
  let mut blocks = vec![
    ...
    paragraph((
      ...
      bold(event.species_name.as_str()),
...
fn group_result_content(catch: &GroupCatchView) -> types::inputMessageRichMessage {
  let mut blocks = vec![
    ...
    table()
      .row(("✨ XP", format_args!("+{}", catch.xp_gained)))
      .row(("⭐ Level", catch.level))
      .compact()
      .into(),
  ];
  ...
  blocks.push(button_row([
    callback_button("📖 Journal", Callback::GroupJournal.encode()),
    callback_button("🏆 Records", Callback::GroupRecords.encode()),
  ]));
  rich(blocks)
}
...
fn group_help_content() -> types::inputMessageRichMessage {
  rich([
    ...
    paragraph("Rustwater is the harbor region where your adventure begins. Your character, discoveries, catches, and gear follow you between private chat and group activities."),
